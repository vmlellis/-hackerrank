...
# Solution: https://www.igvita.com/2009/03/26/ruby-algorithms-sorting-trie-heaps/
...
def rebalance(lowers, highers)
  while !lowers.next.nil? && !highers.next.nil? && lowers.next > highers.next do
    value = lowers.pop
    highers.push(value)
  end
...
def median(lowers, highers)
  return lowers.next.to_f if lowers.size > highers.size
  return lowers.next.to_f if highers.size > lowers.size
  (lowers.next + highers.next) / 2.0
end

lowers = Heap.new { |x, y| (x <=> y) == 1 }
highers = Heap.new { |x, y| (x <=> y) == -1 }
...
n.times do |i|
  ...
  # p "=> #{value}"
  # p [lowers.size, lowers.next]
  # p [highers.size, highers.next]
  # puts median(lowers, highers)
  # puts i
